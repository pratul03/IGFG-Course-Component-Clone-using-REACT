      ...
      <div className="left-0 flex flex-col w-[75%] max-w-5xl mr-auto p-6 text-[#0e0e0e] shadow-lg">
        {array[0]?.topics?.map((item, index) => (
          <div key={index} className="flex flex-col mb-4">
            ...
            <hr className="border-t border-gray-950 my-2" />
            <p className="text-lg">{item.description}</p>
            <span className="flex flex-col justify-center content-center items-center bg-slate-300">
              {item.image && (
                <img
                  src={item.image}
              ...
              {item.imageDescription && (
                <p className="text-[#0e0e0e] italic text-xs font-normal mt-[0.5px]">
                  {item.imageDescription}
                </p>
              )}
            </span>
                      ...
                      {section.langList.map((listItem, listIndex) => (
                        <li key={listIndex}>
                          <Link
                            ...
                            className="text-[20px] text-greenGFG font-medium underline tracking-wide"
                          >
                            {listItem.name}
                          </Link>
                        </li>
