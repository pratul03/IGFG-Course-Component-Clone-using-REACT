...
import { arrayListInJava } from "../../public/arraydata/arrayListInJava";
...
const ArrayListInjavaLang = () => {
  ...
  const handleCodeChange = (key, value) => {
    arrayListInJava[0].topics[0].content =
      arrayListInJava[0].topics[0].content.map((item, idx) =>
  ...
  return (
    <div className="flex w-full flex-col p-6 text-[#0e0e0e]">
      <div className="left-0 ml-[-10px] flex flex-col w-full max-w-5xl mx-auto text-[#0e0e0e] shadow-lg">
        {arrayListInJava[0]?.topics?.map((topic, topicIndex) => (
...
export default ArrayListInjavaLang;
